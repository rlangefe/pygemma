...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--pheno', type=str, help='Phenotype file', required=True)
    parser.add_argument('-c', '--covar', type=str, help='Covariates file', default=None)
    ...
    Y = pd.read_csv(args.pheno, sep=' ', header=None, engine="c").values.astype(np.float32)

    # Remove .bin extension, add .dim
    dim_file = args.geno[:-4] + '.dim'

    with open(dim_file, 'r') as f:
        dim = f.read().split()

    X = X.reshape(int(dim[0]), int(dim[1]))
    X = X.T
    ...
    if args.covar is not None:
        W = pd.read_csv(args.covar, sep=' ', header=None, engine="c").values.astype(np.float32)
    else:
